...
Media Library access is optional. It adds browsing from the device Music library. `Files`, generated tracks, and Sound Lab creations still work without it.

`Music` works with tracks that are truly local to the device: iTunes Store purchases downloaded to the device and audio synced from the Apple Music app on Mac. Synced files are usually preferred because you can choose higher-quality copies such as `ALAC` or other lossless audio. Apple Music subscription downloads currently do not work because they remain DRM-protected subscription copies, even when they appear downloaded in the Music app. Purchased iTunes Store tracks do work.
